...
**R**は統計分析のためのプログラミング言語、**RStudio**はRを便利に使うための編集環境（エディタ）です。両方をインストールし、普段の作業ではRStudioを起動してRを使います。
...
RStudioでは**プロジェクト**を使ってファイルを管理します。プロジェクトを開くと、Rの**作業ディレクトリ**（Rがファイルを探したり保存したりする場所）が自動的にそのフォルダに設定されます。これにより、ファイルのパスを毎回指定する手間がなくなり、分析の管理が楽になります。

::: {.callout-tip}
## 作業ディレクトリの確認
現在の作業ディレクトリは `getwd()` で確認できます。プロジェクトを開いた状態で実行すると、プロジェクトフォルダのパスが表示されます。
...
::: {.callout-tip}
## すでにフォルダを作成済みの場合
上の動画のように、先にフォルダを作成してある場合は、手順2で「New Directory」ではなく「**Existing Directory**」を選択し、作成済みのフォルダを指定してください。
:::

::: {.callout-warning}
## フォルダのパスに日本語を含めない
保存先のパスに日本語が含まれると、ファイルの読み込みでエラーが起きることがあります。例えば `C:/Users/藤原/デスクトップ/benesse_seminar` のようなパスは避け、`C:/Users/fujihara/Desktop/benesse_seminar` のように英数字のパスを選んでください。
:::
...
あるいは、以下のRコードを実行してフォルダを一括作成することもできます。ただし、**プロジェクトを開いた状態**（作業ディレクトリがプロジェクトフォルダに設定されている状態）で実行してください。`getwd()` でプロジェクトフォルダのパスが表示されることを確認してから実行すると安全です。
...
パッケージの利用には2つのステップがあります。

- **`install.packages()`**: パッケージをパソコンにダウンロード・保存する（**初回のみ**）
- **`library()`**: インストール済みのパッケージをRに読み込む（**Rを起動するたびに必要**）

スマートフォンのアプリに例えると、`install.packages()` はアプリのインストール、`library()` はアプリを開く操作に相当します。
:::
...
::: {.callout-warning}
## インストール中の赤いメッセージ
インストール中にコンソールに赤い文字でメッセージが大量に表示されますが、**これはエラーではありません**。パッケージのダウンロード・コンパイルの進捗状況を示すもので、正常な動作です。初めてのインストールでは数分〜10分程度かかることがあります。「パッケージをソースからインストールしますか？」と聞かれたら「No」を選択してください。
:::

::: {.callout-note}
## `library()` 実行時のメッセージ
`library(tidyverse)` を実行すると、含まれるパッケージの一覧や「Conflicts」（関数名の競合）といったメッセージが表示されますが、これも正常です。エラーが出なければ問題ありません。
:::
...
::: {.callout-tip}
## .dtaファイルの使用を推奨
`.dta`（Stata形式）ファイルには、メタデータ（変数の意味を示す変数ラベルや、数値に対応するカテゴリ名を示す値ラベルなど）が含まれているため、こちらの使用を推奨します。`.csv`ファイルにはこれらのメタデータが含まれていません。

ただし、データに付属するメタデータだけを鵜呑みにせず、調査票やコードブック、単純集計表を確認することも重要です。
:::
...
::: {.callout-tip}
## データの中身を見る
読み込んだデータの特定の変数（列）にアクセスするには、`$` を使います。また、`[行, 列]` の形式で特定の行や列を取り出すこともできます。
...
::: {.callout-note}
## CSVファイルを使う場合
`.dta` ではなく `.csv` ファイルを使う場合は、`read_csv()` で読み込みます。

```r
d <- readr::read_csv(here("data", "raw", "1571", "1571.csv"))
```
:::
...
時間があれば、以下の基本操作もぜひ試してみてください。セミナー当日の理解がスムーズになります。
...
`#`（ハッシュ）より右側の文字はRに無視されます。コードの説明や覚え書きとして活用してください。

```{r}
# これはコメントです（実行されません）
1 + 1  # 行の途中からコメントを書くこともできます
```
...
::: {.callout-tip}
## ヘルプの使い方
関数の使い方がわからないときは、`?関数名` で右下ペインにヘルプが表示されます。
